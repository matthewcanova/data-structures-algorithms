...
/**
 * Tests for the QuadProbeHashTable class.
 * 
 * @author Boqian Yao
...
public class QuadProbeHashTableTest {
	...
	static final int NUMBEROFITEMS = 30;
	
	QuadProbeHashTable emptyBadFunctor, emptyMediocreFunctor,
			emptyGoodFunctor, badFunctor, mediocreFunctor, goodFunctor;
	...
	public void setUp() throws Exception {
		...
		emptyBadFunctor = new QuadProbeHashTable(CAPACITY,
				new BadHashFunctor());
		emptyMediocreFunctor = new QuadProbeHashTable(CAPACITY,
				new MediocreHashFunctor());
		emptyGoodFunctor = new QuadProbeHashTable(CAPACITY,
				new GoodHashFunctor());
		
		badFunctor = new QuadProbeHashTable(CAPACITY,
				new BadHashFunctor());
		mediocreFunctor = new QuadProbeHashTable(CAPACITY,
				new MediocreHashFunctor());
		goodFunctor = new QuadProbeHashTable(CAPACITY,
				new GoodHashFunctor());
	...
	public void testSizeNotEmpty() {
		...
		assertEquals(badFunctor.size(), NUMBEROFITEMS);
		assertEquals(mediocreFunctor.size(), NUMBEROFITEMS);
		assertEquals(goodFunctor.size(), NUMBEROFITEMS);
	}
